...
export function centerlineToOutline(centerline: StrokeSample[], settings: BrushSettings): Contour | null {
  ...
  for (let i = 0; i < pts.length; i++) {
    ...
    if (settings.type === "oilBrush" && (settings.jitter ?? 0) > 0) {
      ...
      // Light, independent per-edge grain: each side gets its own small
      // pseudo-random nudge along the nib's radial direction, using
      // different seeds per side so the two edges don't bulge/shrink in
      // lockstep (which would just look like a wobbly-but-smooth line).
      // Kept fine and low-amplitude — a "touch" of unevenness, distinct
      // from the interior counter-holes handled in
      // roughBrushOutlineContours() and from Grunge's much heavier spikes.
      const roughJitter = settings.jitter ?? 0;
      const mag = Math.hypot(vx, vy) || 1;
      const ux = vx / mag;
      const uy = vy / mag;
      const leftNoise = pseudoNoise(i * 17.23 + 3.7) * 0.6 + pseudoNoise(i * 41.9 + 3.7) * 0.4;
      const rightNoise = pseudoNoise(i * 13.71 + 91.4) * 0.6 + pseudoNoise(i * 37.2 + 91.4) * 0.4;
      const leftMag = Math.max(mag * 0.75, mag + leftNoise * roughJitter * semiMajor * 0.4);
      const rightMag = Math.max(mag * 0.75, mag + rightNoise * roughJitter * semiMajor * 0.4);
