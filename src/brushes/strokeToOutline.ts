...
export function centerlineToOutline(centerline: StrokeSample[], settings: BrushSettings): Contour | null {
  ...
  for (let i = 0; i < pts.length; i++) {
    ...
    if (settings.type === "oilBrush" && (settings.jitter ?? 0) > 0) {
      ...
      // heavily weighted toward both ends via `endBoost`, matching a real
      // brush's lift-off — replace the smooth boundary point with 2-3
      // splayed prongs of uneven length, angled slightly along the stroke
      // direction so they read as combed/dragged bristles rather than a
      // radial burst.
      const endBoost = 1 + 4 * Math.max(0, 0.3 - Math.min(s, 1 - s)) / 0.3;
      const frayChance = 0.028 * endBoost;
      ...
      if (
        ...
        const baseLen = semiMajor * (0.7 + 0.9 * endBoost) * (0.6 + ((pseudoNoise(seed) + 1) / 2));
        for (let p = 0; p < prongCount; p++) {
          ...
          const drift = spread * semiMajor * 0.9 + pseudoNoise(seed + p * 11.3) * semiMajor * 0.25;
          left.push({
            x: leftBaseP.x + ux * (prongLen * (p % 2 === 0 ? 1 : 0.55)) + tx * drift,
            y: leftBaseP.y + uy * (prongLen * (p % 2 === 0 ? 1 : 0.55)) + ty * drift,
          });
          // Pull back toward baseline between prongs so each finger reads
          // as a distinct claw rather than one wide wedge.
          if (p < prongCount - 1) {
            left.push({
              x: leftBaseP.x + ux * mag * 0.25 + tx * drift * 0.4,
              y: leftBaseP.y + uy * mag * 0.25 + ty * drift * 0.4,
            });
          }
        }
        leftFrayCooldown = 3 + Math.round(((pseudoNoise(seed + 99) + 1) / 2) * 4);
      } else {
        left.push(leftBaseP);
      }
      ...
      if (
        ...
        for (let p = 0; p < prongCount; p++) {
          ...
          const drift = spread * semiMajor * 0.9 + pseudoNoise(seed + p * 11.3) * semiMajor * 0.25;
          right.push({
            x: rightBaseP.x - ux * (prongLen * (p % 2 === 0 ? 1 : 0.55)) + tx * drift,
            y: rightBaseP.y - uy * (prongLen * (p % 2 === 0 ? 1 : 0.55)) + ty * drift,
          });
          if (p < prongCount - 1) {
            right.push({
              x: rightBaseP.x - ux * mag * 0.25 + tx * drift * 0.4,
              y: rightBaseP.y - uy * mag * 0.25 + ty * drift * 0.4,
            });
          }
        }
        rightFrayCooldown = 3 + Math.round(((pseudoNoise(seed + 99) + 1) / 2) * 4);
      } else {
...
function makeRoughHole(center: Point, radius: number, seed: number, desiredSign: number): Contour {
  const sides = 5 + Math.round(((pseudoNoise(seed + 5.5) + 1) / 2) * 4); // 5..9 sides
  ...
  for (let i = 0; i < sides; i++) {
    ...
    const wobble = 1 + pseudoNoise(seed + i * 3.7) * 0.5;
 ...
 * so the texture doesn't flicker on re-render, and are kept few/small per
 * the brush's "Rough" character (a light scatter of tiny voids, not a
 * heavily distressed edge like Grunge).
 */
function roughBrushOutlineContours(centerline: StrokeSample[], settings: BrushSettings): Contour[] {
  ...
  const holeCount = totalLength <= margin * 2 ? 0 : Math.max(0, Math.min(48, Math.round((totalLength / 85) * density)));

  const holes: Contour[] = [];
  for (let h = 0; h < holeCount; h++) {
    const seed = h * 971.31 + totalLength * 0.013;
    const tTarget = margin + ((pseudoNoise(seed) + 1) / 2) * (totalLength - margin * 2);
    let idx = 1;
    ...
    const taper = taperFactor(s, settings.taperStart, settings.taperEnd);
    const edgeGuard = Math.max(1.2, halfWidth * 0.16);
    const usableHalf = Math.max(1, halfWidth * taper - edgeGuard);
    const offset = pseudoNoise(seed + 41.7) * usableHalf * 0.78;
    const center = { x: p1.x + normal.x * offset, y: p1.y + normal.y * offset };
    // Wider size range than before (~0.4x..2.0x) so the scatter mixes tiny
    // pinholes with a few noticeably larger torn-out chunks — the "abstract"
    // look, rather than a uniform dot pattern.
    const sizeJitter = 0.4 + ((pseudoNoise(seed + 88.2) + 1) / 2) * 1.6;
    const radius = Math.max(0.5, halfWidth * (settings.holeSize ?? 0.16) * sizeJitter);
    if (radius >= usableHalf) continue; // stays fully inside the stroke body
