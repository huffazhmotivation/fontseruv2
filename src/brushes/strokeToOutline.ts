...
export function centerlineToOutline(centerline: StrokeSample[], settings: BrushSettings): Contour | null {
  ...
  for (let i = 0; i < pts.length; i++) {
    ...
    const leftBase = { x: pts[i].x + normal.x * r, y: pts[i].y + normal.y * r };
    const rightBase = { x: pts[i].x - normal.x * r, y: pts[i].y - normal.y * r };
    ...
    if (settings.type === "grunge") {
      ...
      left.push(leftBase);
      left.push({
        x: pts[i].x + normal.x * r * leftSpike + tx * leftNoise * tangentJitter,
        y: pts[i].y + normal.y * r * leftSpike + ty * leftNoise * tangentJitter,
      });

      right.push(rightBase);
      right.push({
        x: pts[i].x - normal.x * r * rightSpike + tx * rightNoise * tangentJitter,
        y: pts[i].y - normal.y * r * rightSpike + ty * rightNoise * tangentJitter,
      });
    } else {
      left.push(leftBase);
      right.push(rightBase);
 ...
 * + winding). Built as a wobbly polygon rather than a perfect circle so a
 * cluster of holes reads as organic texture, not a row of identical dots.
 */
function makeRoughHole(center: Point, radius: number, seed: number, desiredSign: number): Contour {
  ...
  for (let i = 0; i < sides; i++) {
    ...
    const wobble = 1 + pseudoNoise(seed + i * 3.7) * 0.35;
...
function roughBrushOutlineContours(centerline: StrokeSample[], settings: BrushSettings): Contour[] {
  ...
  const holeCount = totalLength <= margin * 2 ? 0 : Math.max(0, Math.min(10, Math.round((totalLength / 120) * density)));
  ...
  for (let h = 0; h < holeCount; h++) {
    ...
    const taper = taperFactor(s, settings.taperStart, settings.taperEnd);
    const edgeGuard = Math.max(1.5, halfWidth * 0.22);
    const usableHalf = Math.max(1, halfWidth * taper - edgeGuard);
    const offset = pseudoNoise(seed + 41.7) * usableHalf * 0.7;
    const center = { x: p1.x + normal.x * offset, y: p1.y + normal.y * offset };
    const sizeJitter = 0.6 + ((pseudoNoise(seed + 88.2) + 1) / 2) * 0.7; // ~0.6x .. 1.3x
    const radius = Math.max(0.6, halfWidth * (settings.holeSize ?? 0.16) * sizeJitter);
