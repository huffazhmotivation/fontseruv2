...
export const BRUSH_PRESETS: Record<BrushType, BrushPreset> = {
  ...
  rough: {
    id: "rough",
    label: "Rough",
    description: "Constant-width line like Monoline, with a few small irregular texture holes inside the stroke.",
    settings: {
      size: 20,
      minSize: 20,
      maxSize: 20,
      ...
      roundness: 1,
      angle: 0,
      // A gentle taper (not 0 like Monoline) so the ends don't read as a
      // perfectly flat-cut block — still reads as "constant width" mid-stroke.
      taperStart: 0.06,
      taperEnd: 0.06,
      pressureEnabled: false,
      // Deliberately sparse and small — see roughBrushOutlineContours() in
      // strokeToOutline.ts. This is a real vector counter (hole) per dot,
      // not a raster texture, so it exports cleanly into the font too.
      holeDensity: 0.9,
      holeSize: 0.16,
    },
  },
