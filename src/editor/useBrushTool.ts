 ...
 *
 * Uses a "pulled string" (leash) model instead of a plain exponential
 * average: the live point only moves once the raw pointer strays past a
 * strength-scaled radius, then eases forward just enough to stay at that
 * radius. Below that radius the point holds perfectly still, which removes
 * hand tremor completely at low speed — a plain low-pass filter still lets
 * small jitter drift through a little on every sample. Above it, the point
 * tracks deliberate motion directly along the straight line to the cursor
 * rather than lagging behind it, which is what keeps fast curved strokes
 * looking like clean arcs instead of rounded-off, corner-cut facets.
 */
export function stabilizeBrushPoint(p: Point, previous: Point | null, amount: number): Point {
  ...
  const leash = 22 * strength;
  const dx = p.x - previous.x;
  const dy = p.y - previous.y;
  const dist = Math.hypot(dx, dy);
  if (dist <= leash) return previous;
